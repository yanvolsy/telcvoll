...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    const { rows: students } = await pool.query(
      `SELECT s.id, s.name, s.first_name, s.last_name, s.email, s.phone, s.country,
              COALESCE(s.auth_provider, 'email') AS auth_provider,
              s.email_verified, s.is_blocked, s.is_paid, s.created_at, s.last_login_at,
              COUNT(DISTINCT a.id)::int AS attempts,
              COALESCE(ROUND(AVG(a.percent), 1), 0) AS avg_score,
              sub.plan_name,
              sub.plan_id,
              sub.expires_at,
              CASE WHEN (s.is_paid = TRUE OR sub.expires_at > NOW()) THEN TRUE ELSE FALSE END AS is_subscription_active
       FROM students s
