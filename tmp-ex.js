...
function renderMatching(){
 const scrollState={main:document.querySelector('.exam-main')?.scrollTop||0,side:document.querySelector('.exam-side')?.scrollTop||0,workspace:document.querySelector('.exam-workspace')?.scrollTop||0,x:window.scrollX||0,y:window.scrollY||0};
 const restoreScroll=()=>{const restore=()=>{const m=document.querySelector('.exam-main'),sd=document.querySelector('.exam-side'),ws=document.querySelector('.exam-workspace');if(m)m.scrollTop=scrollState.main;if(sd)sd.scrollTop=scrollState.side;if(ws)ws.scrollTop=scrollState.workspace;window.scrollTo(scrollState.x,scrollState.y);};restore();requestAnimationFrame(()=>{restore();requestAnimationFrame(restore);});setTimeout(restore,40);};
 ...
 const resultHtml=resultData?`<div class="side-result ${resultData.percent>=60?'result-pass':'result-fail'}"><strong>${resultData.score}/${resultData.max}</strong><span>${resultData.percent}% — ${esc(resultData.result)}</span></div>`:'';
 document.getElementById('content').innerHTML=layout(main,`<div class="side-result-slot">${resultHtml}</div><div class="side-title side-title-large"><div><b>ÜBERSCHRIFTEN</b><small>FÜR TEXT</small></div><span>${answeredCount}/${items.length}</span></div><div class="matching-controls"><button type="button" class="side-icon shuffle-all" aria-label="${text('خلط العناوين والفقرات','Überschriften und Texte mischen')}" title="${text('خلط العناوين والفقرات','Überschriften und Texte mischen')}">⤨</button></div><div class="heading-list">${side}</div>`);
 ...
 document.querySelectorAll('[data-paragraph-select]').forEach(sel=>sel.onchange=e=>{
   ...
   renderMatching();restoreScroll();bindActions();
 });
 document.querySelectorAll('[data-clear]').forEach(b=>b.onclick=e=>{e.stopPropagation();delete answers[String(b.dataset.clear)];activeItem=String(b.dataset.clear);renderMatching();restoreScroll();bindActions();});
 document.querySelectorAll('[data-choice]').forEach(b=>{b.onmousedown=e=>e.preventDefault();b.onclick=e=>{e.preventDefault();if(b.disabled||!activeItem)return;const chosen=b.dataset.choice;Object.keys(answers).forEach(k=>{if(k!==String(activeItem)&&answers[k]===chosen)delete answers[k];});answers[String(activeItem)]=chosen;renderMatching();restoreScroll();bindActions();};});
 const sh=document.querySelector('.shuffle-all');
 if(sh)sh.onclick=e=>{e.stopPropagation();matchingOrder=shuffle(matchingOrder);headingOrder=shuffle(headingOrder);activeItem=null;renderMatching();restoreScroll();bindActions();};
 bindTextTools();
}
function renderSprachbausteine1(){
 ...
 const sh=document.querySelector('.shuffle-spb1'); if(sh)sh.onclick=e=>{e.preventDefault();e.stopPropagation();items.forEach(it=>{const n=String(it.position_no);optionOrders[n]=shuffle((it.options||[]).slice(0,3).map(o=>o.option_key));});renderWorkspace(false);bindActions();};
 bindTextTools();
}
...
function renderChoiceQuestions(){
 ...
 if(isLesen2){
   ...
   if(sh)sh.onclick=e=>{
     ...
     renderWorkspace(false); bindActions();
   };
 }
 bindTextTools();
}

function renderBody(body){let html=escBody(body||'');(model.items||[]).forEach(i=>{const n=String(i.position_no);const val=answerValue(n);const isSpb1=model.exercise.section==='Sprachbausteine'&&model.exercise.teil==='Teil 1';const isSpb2=model.exercise.section==='Sprachbausteine'&&model.exercise.teil==='Teil 2';const examNo=isSpb1?Number(n)+20:isSpb2?Number(n)+30:Number(n);let re;if(isSpb1){const correctText=String(i.correctText||i.prompt||'').trim();re=correctText?new RegExp('\\('+examNo+'\\)|\\['+examNo+'\\]|\\{'+examNo+'\\}|\\['+escapeRegExp(correctText)+'\\]','g'):new RegExp('\\('+examNo+'\\)|\\['+examNo+'\\]|\\{'+examNo+'\\}','g');}else{re=new RegExp('\\('+n+'\\)|\\['+n+'\\]|\\{'+n+'\\}','g');}html=html.replace(re,`<span class="inline-answer ${val?'filled':''}">${val?esc(labelFor(n,val)):'('+examNo+')'}</span>`);});return html;}function escapeRegExp(s){return s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');}function selectAnswer(n,val){activeItem=String(n);answers[String(n)]=val;renderWorkspace(false);bindActions();}
...
function renderSpeaking(){
 ...
 initInlineSpeaking();bindTextTools();
}
function initInlineSpeaking(){
...
function bindActions(){
 ...
 if(mb)mb.onclick=async()=>{
   try{
     ...
     bindActions();
   }catch(e){document.getElementById('content').insertAdjacentHTML('beforeend',`<div class="alert bad">${esc(e.message)}</div>`);}
 };
...
function renderResult(r){submitted=true;resultData=r;const detailByPrompt=new Map((r.details||[]).map(d=>[String(d.prompt),d]));document.querySelectorAll('.exam-text-card').forEach(card=>{const idn=card.dataset.item;const it=model.items.find(x=>String(x.position_no)===String(idn));const d=r.details?.find(x=>String(x.prompt)===String(it?.prompt));if(d){card.classList.remove('answer-correct','answer-wrong');card.classList.add(d.ok?'answer-correct':'answer-wrong');}});const side=document.querySelector('.exam-side');if(side&&model.exercise.section==='Lesen'&&model.exercise.teil==='Teil 1'){renderMatching();bindActions();}else{const rows=(r.details||[]).map(d=>`<div class="result-row ${d.ok?'ok':'bad'}"><b>${d.ok?text('صحيح','Richtig'):text('خطأ','Falsch')}</b><span>${esc(d.prompt)}</span>${!d.ok?`<small>${text('الحل: ','Lösung: ')}${esc((model.items.find(x=>String(x.prompt)===String(d.prompt))?.options||[]).find(o=>String(o.option_key)===String(d.correct_answer))?.option_text||d.correct_answer||'')}</small>`:''}</div>`).join('');document.getElementById('content').insertAdjacentHTML('afterbegin',`<div class="inline-result-panel ${r.percent>=60?'pass':'fail'}"><strong>${r.score} / ${r.max}</strong><span>${r.percent}% — ${esc(r.result)}</span><div>${rows}</div></div>`);bindActions();}}
