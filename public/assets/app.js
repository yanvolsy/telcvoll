...
const TELC_ENTERED_KEY = 'telc_entered_v1';
function getEnteredExerciseIds() {
  try {
    const raw = localStorage.getItem(TELC_ENTERED_KEY);
    return raw ? new Set(JSON.parse(raw).map(String)) : new Set();
  } catch { return new Set(); }
}
function markExerciseEntered(exerciseId) {
  if (!exerciseId) return;
  try {
    const s = getEnteredExerciseIds();
    s.add(String(exerciseId));
    localStorage.setItem(TELC_ENTERED_KEY, JSON.stringify([...s]));
  } catch {}
}
function getExerciseResult(exerciseId) {
  if (!exerciseId) return null;
  try {
    const r1 = localStorage.getItem('last_exercise_result_' + exerciseId);
    if (r1) return JSON.parse(r1);
    const r2 = localStorage.getItem('writing_ai_result_' + exerciseId);
    if (r2) return JSON.parse(r2);
  } catch {}
  return null;
}
function getExerciseStatus(exerciseId, attemptsMap = {}) {
  const idStr = String(exerciseId);
  const srv = attemptsMap[idStr];
  if (srv) {
    const pct = srv.percent != null ? Number(srv.percent) : (srv.max_score ? Math.round(Number(srv.score || 0) / Number(srv.max_score) * 100) : 0);
    const passed = (srv.result === 'Bestanden') || pct >= 60;
    return {
      status: passed ? 'passed' : 'failed',
      percent: Math.round(pct),
      score: srv.score,
      max: srv.max_score,
      labelAr: passed ? `ناجح (${Math.round(pct)}%)` : `يحتاج إعادة (${Math.round(pct)}%)`,
      labelDe: passed ? `Bestanden (${Math.round(pct)}%)` : `Nicht bestanden (${Math.round(pct)}%)`
    };
  }
  const local = getExerciseResult(exerciseId);
  if (local) {
    let pct = 0;
    if (local.percent != null) pct = Number(local.percent);
    else if (local.max) pct = Math.round((Number(local.score || 0) / Number(local.max)) * 100);
    else if (local.overall_score != null) pct = Math.round((Number(local.overall_score) / (Number(local.max_score) || 45)) * 100);
    else if (local.total != null) pct = Math.round((Number(local.total) / 25) * 100);
    const passed = (local.result === 'Bestanden') || pct >= 60;
    return {
      status: passed ? 'passed' : 'failed',
      percent: Math.round(pct),
      score: local.score ?? local.overall_score ?? local.total,
      max: local.max ?? local.max_score,
      labelAr: passed ? `ناجح (${Math.round(pct)}%)` : `يحتاج إعادة (${Math.round(pct)}%)`,
      labelDe: passed ? `Bestanden (${Math.round(pct)}%)` : `Nicht bestanden (${Math.round(pct)}%)`
    };
  }
  if (isExerciseCompleted(exerciseId)) {
    return { status: 'passed', percent: 100, labelAr: 'مكتمل', labelDe: 'Bestanden' };
  }
  if (getEnteredExerciseIds().has(idStr)) {
    return { status: 'in-progress', labelAr: 'قيد التدريب', labelDe: 'In Bearbeitung' };
  }
  return { status: 'new', labelAr: 'جديد', labelDe: 'Neu' };
}
