...
let html = fs.readFileSync('public/plans.html', 'utf8');

// 1. Add banner slot before plansGrid
if (!html.includes('id="plansRenewalBannerSlot"')) {
  html = html.replace('<div id="plansGrid" class="plans-grid-wrap">', '<div id="plansRenewalBannerSlot"></div>\n  <div id="plansGrid" class="plans-grid-wrap">');
  console.log('Banner slot added to plans.html');
}

// 2. Update script variables and functions
const oldScriptStart = `let currentUser = null;
const checkoutModal = document.getElementById('checkoutModal');`;

const newScriptStart = `let currentUser = null;
...
if (html.includes(oldScriptStart)) {
  ...
  console.log('Script variables updated in plans.html');
}

// 3. Update openCheckout to check remainingDays
...
if (html.includes(oldOpenCheckout)) {
  ...
  console.log('openCheckout guarded in plans.html');
}

// 4. Update loadPlans and user initialization
const oldLoadPlansEnd = `    grid.innerHTML = plans.map(p => {
      const isFree = isPlanFree(p);
      const isFeatured = p.is_featured || (!isFree && p.duration_days === 30);
      ...
      return \`
        <article class="plans-card \${isFeatured ? 'featured' : ''}">
          ...
          <div class="plans-card-header">
            ...
            <p class="muted" style="font-size:13.5px;margin:8px 0 0;line-height:1.6;">\${p.description || 'وصول فوري وشامل لكافة التمارين والامتحانات والمساعد الذكي.'}</p>
          </div>
...
const newLoadPlansEnd = `    // Render Active Subscription Guard Banner if applicable
...
if (html.includes(oldLoadPlansEnd)) {
  html = html.replace(oldLoadPlansEnd, newLoadPlansEnd);
  console.log('loadPlans updated in plans.html');
}

// 5. Update user init at bottom of plans.html
const oldUserInit = `  try {
...
if (html.includes(oldUserInit)) {
  html = html.replace(oldUserInit, newUserInit);
  console.log('userInit updated in plans.html');
}

fs.writeFileSync('public/plans.html', html, 'utf8');
console.log('public/plans.html successfully saved!');
