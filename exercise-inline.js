...
function parseSpeakingTasks(body){const raw=String(body||'').replace(/\r/g,'');const settings=parseSettings(model.exercise);const candidates=[];const src=Array.isArray(settings.tasks)?settings.tasks:Array.isArray(settings.aufgaben)?settings.aufgaben:[];src.forEach(x=>{const v=typeof x==='string'?x:(x?.text||x?.title||'');if(v.trim())candidates.push(v.trim())});if(!candidates.length)raw.split('\n').map(x=>x.trim()).filter(Boolean).forEach(line=>{const m=line.match(/^(?:[-•]\s*)?(\d{1,2})[.)]\s*(.+)$/);if(m)candidates.push(m[2].trim())});return [...new Set(candidates)].slice(0,12)}
function renderSpeakingTaskList(body){const tasks=parseSpeakingTasks(body);if(!tasks.length)return '';return `<section class="speaking-tasks-panel"><div class="speaking-section-head"><div><span class="speaking-section-kicker">${text('الخطوات والمطلوب','Aufgaben')}</span><h2>${text('خطوات المهمة','Aufgaben')}</h2></div><button type="button" class="mini-tool" data-toggle-speaking-tasks aria-expanded="true">▾</button></div><div class="speaking-task-list" data-speaking-task-list>${tasks.map((task,i)=>{const cid='sp-task-'+(i+1);return `<article class="speaking-task-item"><span class="speaking-task-number">${i+1}</span><div class="speaking-task-item-body"><div class="speaking-task-item-top"><b id="${cid}" data-content-id="${cid}" data-source="${esc(task)}">${esc(task)}</b>${textTools(task,cid)}</div><div class="inline-translation" id="${cid}-translation"></div></div></article>`}).join('')}</div></section>`}
